...
export default function RewardToggleRow({ catalog }: { catalog: any[] }) {
  // Local state to simulate toggles
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-12">
      <div className="px-6 py-5 border-b border-gray-100 bg-gray-50/50">
        ...
        <p className="text-sm text-brand-text">Gerencie quais prêmios estarão visíveis no Clube de Benefícios da sua empresa.</p>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-100">
            {items.map((item) => (
              <tr key={item.id} className={`transition-colors ${item.active ? 'hover:bg-gray-50/50' : 'bg-gray-50 opacity-60'}`}>
                <td className="px-6 py-4">
                  <div className="font-bold text-brand-slate">{item.title}</div>
                  <div className="text-xs font-semibold text-brand-text mt-1 uppercase tracking-wide">Fornecedor: {item.partner}</div>
                </td>
