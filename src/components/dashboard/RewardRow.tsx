  ...
  return (
    <div className="relative px-8 md:px-16 mb-12 py-8 bg-gradient-to-r from-gray-50 to-white border-y border-gray-100">
      ...
      <div className="flex gap-6 overflow-x-auto hide-scrollbar pb-4 -mx-4 px-4">
        {rewards.map((reward) => {
          const price = reward.pricePoints ?? reward.pointsRequired ?? 0;
          const missing = Math.max(0, price - userPoints);
          const progress = price > 0 ? Math.min(100, (userPoints / price) * 100) : 100;
            ...
            >
              <div className="flex h-32">
                <div className="w-1/3 bg-gray-100 relative">
                  <img
                    src={reward.thumbnail || reward.imageUrl}
                    alt={reward.title}
                ...
                <div className="w-2/3 p-4 flex flex-col justify-between">
                  <div>
                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                      {reward.brand || reward.partnerStore || 'Parceiro'}
                    </span>
                    <h3 className="font-bold text-brand-slate text-sm leading-tight mt-1">{reward.title}</h3>
                  </div>

                  <div className="flex items-center justify-between mt-2">
                    <div className="text-emerald-600 font-bold text-sm">{price} pts</div>
                    {missing > 0 && (
